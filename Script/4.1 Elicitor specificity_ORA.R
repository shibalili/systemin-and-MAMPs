...
#### plot y-axis: more detailed PTI protein groups #####
sp <- rbind(Top_s,Top_f,Top_chi)%>%filter(Rank_short<=5,super_class=="Kinase")%>%select(-c(1,4,9:10))
#sp <- rbind(Top_s,Top_f,Top_chi)%>%filter(Rank_short<=5,super_class=="PPase")%>%select(-c(1,4,9:10))
#sp <- rbind(Top_s,Top_f,Top_chi)%>%filter(Rank_short<=5,super_class=="Transporter")%>%select(-c(1,4,9:10))
sp <- sp[!duplicated(sp),]
sp$super_class <- as.character(sp$super_class) # remove factor
table(sp$super_class1)
sp_df <- plyr::count(sp, vars =c("super_class1","sig_cluster","sig_treatment"))%>%filter(super_class1!="0")
sp_df$sig_treatment <- factor(sp_df$sig_treatment,levels = c("systemin","flg22","chitin"))
ggplot(sp_df,aes(sig_cluster,super_class1))+geom_point(aes(color=freq,size=freq),show.legend = T)+scale_y_discrete(limits=rev)+
  scale_x_discrete(labels=c("1","2","3","4","5","6","7"))+facet_grid(~as.factor(sig_treatment))+
  guides(color= guide_legend(), size=guide_legend())+scale_size_area(max_size = 8)+
...
############### skip ORA directly plot PTI class in sp #########################
sp_s <- readRDS("../SFCH/Data/sp_sys1563.rds")%>%select(c(1,3:7,15))%>%mutate(sig_treatment="systemin",.before = 1)
colnames(sp_s)[8] <- "cluster"
sp_f <- readRDS("../SFCH/Data/sp_flg1332.rds")%>%select(c(1,3:7,16))%>%mutate(sig_treatment="flg22",.before = 1)
colnames(sp_f)[8] <- "cluster"
sp_c <- readRDS("../SFCH/Data/sp_chi1499.rds")%>%select(c(1,3:7,17))%>%mutate(sig_treatment="chitin",.before = 1)
colnames(sp_c)[8] <- "cluster"
sp <- rbind(sp_s,sp_f,sp_c)%>%filter(cluster!="filtered")
sp$super_class <- as.character(sp$super_class) # remove factor
rownames(sp) <- NULL
sp_df <- plyr::count(sp, vars =c("super_class","super_class1","cluster","sig_treatment"))%>%filter(super_class=="Transporter",super_class1!="0")
sp_df$sig_treatment <- factor(sp_df$sig_treatment,levels = c("systemin","flg22","chitin"))
ggplot(sp_df,aes(cluster,super_class1))+geom_point(aes(color=freq,size=freq),show.legend = T)+scale_y_discrete(limits=rev)+
  scale_x_discrete(labels=c("1","2","3","4","5","6","7"))+facet_grid(~as.factor(sig_treatment))+
  guides(color= guide_legend(), size=guide_legend())+scale_size_area(max_size = 8)+
  geom_text(position = "identity",aes(label=freq),colour="white",size =2.8)+
  labs(y='',x='Phosphorylation clusters',size='P_sites number',color='P_sites number')+
...
######### history ###########
ggplot(sp_df,aes(sig_cluster,bin.name))+geom_point(aes(color=cut(pvalue, c(-Inf, 0.05, Inf)),size=freq),show.legend = T)+scale_y_discrete(limits=rev)+scale_x_discrete(labels=c("1","2","3","4","5","6","7"))+
  facet_grid(~as.factor(sig_treatment))+
  scale_color_manual(name = "pvalue",values = c("(-Inf,0.05]" = "#371ea3","(0.05, Inf]" = "#b3eebe"),labels = c("<0.05", ">=0.05"))+
  scale_size_area(max_size = 8)+labs(y='',x='Phosphorylation clusters',size='P_sites number')+
  theme(axis.title=element_text(size=10,colour = 'black'), axis.text=element_text(size=10,colour = 'black'), 
        panel.background = element_rect(color='black',linewidth = 0.5,fill = "transparent"), 
        plot.background = element_rect(fill = "transparent", color = NA),
